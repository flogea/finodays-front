...
async function getOperationsIn(address) {
  const web3 = new Web3(RPC_LINK);

  const token1Contract = new web3.eth.Contract(ADD_MERO_ABI, ADD_MERO_ADDRESS1);
  const currency1 = await token1Contract.methods.getOperationsInArrayByAddress(address).call();
...
export default getOperationsIn;
